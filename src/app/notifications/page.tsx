...
const NotificationsPage = () => {
    ...
    const fetchNotifications = async () => {
        ...
        try {
            ...
            const normalizedData: Notification[] = rawData
                .map((item: any) => {
                    let normalizedStatus: NotificationStatus = 'pending';

                    // Check both entityStatus and bookingStatus
                    const rawStatus = (item.entityStatus || item.bookingStatus || '').toLowerCase();
    ...
    const renderStatusBadge = (status: NotificationStatus) => {
        ...
        if (!statusKey || !STATUS_CONFIG[statusKey]) {
            return (
                <span className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-400/10">
                    <Clock size={12} />
                    Unknown
                </span>
            );
        ...
        const style = STATUS_CONFIG[statusKey];
    ...
    const handleNotificationClick = (item: Notification) => {
        ...
        if (item.entity_type === 'online_collection') {
            router.push('/onlinecollection');
        } else if (item.entity_type === 'booking' || item.type === 'BOOKING_CREATED') {
            router.push('/booking');
        } else {
            router.push('/lead');
        }
