...
const LeadComponent: React.FC = () => {
    ...
    // Get lead permissions from role
    ...
    const filters: FilterConfig<any>[] = [
        ...
        { key: 'leadStatus', label: 'Lead Status', type: 'multiSelect', placeholder: 'Select lead status...' },
        { key: 'leadStage', label: 'Lead Stage', type: 'multiSelect', placeholder: 'Select lead stage...' },
        { key: 'platformType', label: 'Platform Type', type: 'multiSelect', placeholder: 'Select platform type...' },
        { key: 'assignedUserName', label: 'Assigned Person', type: 'select', placeholder: 'Select assigned person...' },
        { key: 'city', label: 'City', type: 'multiSelect', placeholder: 'Select cities...' },
        { key: 'state', label: 'State', type: 'multiSelect', placeholder: 'Select states...' },
        { key: 'plotNumber', label: 'Plot Number', type: 'text', placeholder: 'Search by plot number...' },
        { key: 'plotPrice', label: 'Plot Price', type: 'text', placeholder: 'Search by plot price...' },
    ...
    const handleSaveLead = async (data: any) => {
        ...
        try {
            ...
            toast.success(currentLead ? 'Lead updated successfully' : 'Lead added successfully');
            setIsModalOpen(false);
            dispatch(fetchLeads({ page: currentPage, limit: pageSize, searchValue }));
        } catch {
            toast.error(currentLead ? 'Failed to update lead' : 'Failed to add lead');
        } finally {
            setIsSaving(false);
        }
    };
        ...
        <div className="space-y-8 p-3 sm:p-6 dark:bg-gray-900 dark:text-gray-100">
            ...
            <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300">Manage leads with advanced filtering</p>
                </div>
                <div className="flex gap-3">
                    {hasPermission(24, "export") && (
                        <label onClick={handleExport} className="flex items-center justify-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-sm sm:text-base
                    ...
                    {hasPermission(20, "upload") && (
                        <label className="flex items-center justify-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg bg-green-500 hover:bg-green-600 dark:bg-green-700 dark:hover:bg-green-800 text-white cursor-pointer">
                    ...
                    {hasPermission(21, "add") && (
                        <label onClick={handleAdd} className="flex items-center justify-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg bg-blue-500 hover:bg-blue-600 dark:bg-blue-700 dark:hover:bg-blue-800 text-white cursor-pointer">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-x-auto">
                <style jsx>{`
                    ...
                    .fresh-lead-blink { animation: greenBlink 2s infinite; transition: all 0.3s ease; }
                    .fresh-lead-blink:hover { animation-play-state: paused; background-color: rgba(34,197,94,0.15) !important; }
                `}</style>
                <CustomTable<any>
                    data={sortedData}
                    columns={columns}
                    ...
                    actions={(row) => (
                        <div className="flex gap-2">
                            {hasPermission(22, 'edit') && (
                                <button onClick={() => handleEdit(row)} title="Edit" className="p-1 rounded text-blue-500 dark:text-blue-300 hover:text-blue-700 dark:hover:text-blue-500 cursor-pointer"><Pencil className="w-4 h-4" /></button>
                            )}
                            {hasPermission(4, 'delete') && (
                                <button onClick={() => handleDelete(row)} title="Delete" className="p-1 rounded text-red-500 dark:text-red-300 hover:text-red-700 dark:hover:text-red-500 cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                            )}
                            <button onClick={() => { setSelectedLeadId(row); setIsFollowUpModalOpen(true); }} title="Follow Up" className="text-green-500 dark:text-green-300 hover:text-green-700 dark:hover:text-green-500 p-1 cursor-pointer">📞</button>
                        </div>
                    )}
                />
            </div>
            ...
            {/* Modals */}
            <ComprehensiveLeadModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSaveLead} initialData={currentLead} isLoading={isSaving} />
            {selectedLeadId && <FollowUpLeadModal isOpen={isFollowUpModalOpen} onClose={() => setIsFollowUpModalOpen(false)} lead={selectedLeadId} />}
            <TimelineLeadModal isOpen={isTimelineModalOpen} onClose={() => setIsTimelineModalOpen(false)} lead={currentTimelineLead} />
            <DeleteConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onDelete={confirmDelete} title="Confirm Deletion" message={`Are you sure you want to delete "${leadToDelete?.name}"?`} Icon={Trash2} />
            <UploadPreviewModal isOpen={isUploadPreviewOpen} onClose={handleClosePreview} onConfirmUpload={handleConfirmUpload} fileName={fileName} previewData={previewData} isLoading={uploadLoading} totalRows={previewData.length} />
            <ExportModal isOpen={isExportModalOpen} onClose={() => setIsExportModalOpen(false)} data={sortedData} fileName={`leads_export_${new Date().toISOString().split('T')[0]}`} columns={columns} />
        </div>
    );
};
