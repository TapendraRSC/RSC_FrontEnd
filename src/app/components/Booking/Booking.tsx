...
interface BookingData {
    ...
    paymentPlatformId?: number | string;
...
const BookingComponent: React.FC = () => {
    ...
    const fetchUsers = useCallback(async () => {
        try {
            const token = getAuthToken();
            const res = await fetch(`${API_BASE_URL}/users/getAllUser?page=1&limit=10000`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            const json = await res.json();
            if (json?.data?.data) setUsers(json.data.data);
        } catch (err) {
    ...
    const createdByOptions = useMemo(() =>
        users.map(u => ({ id: u.id, name: `${u.name} (${u.roleType})` })),
        [users]
    );
    ...
    const fetchBookingsData = useCallback(async () => {
        const token = getAuthToken();
        if (!token) {
            setBookingList([]); setTotal(0); setTotalPages(1);
            return;
        }
        setLoading(true);
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/bookings/getAllBookings?${queryParams.toString()}`, {
                method: 'GET',
    ...
    const transformBookingsForTable = useMemo((): any[] => {
        return (bookingList || []).map((booking: BookingData) => {
            ...
            return {
                ...
                payment_platform_id: booking.payment_platform_id || (booking as any).paymentPlatformId || '',
                ...
                plotId: booking.plotId || null,
                plotNumber: booking.plotNumber || 'N/A',
                plotSize: booking.plotSize || '',

                price: booking.price || booking.totalPlotAmount || '',
                onlinePrice: booking.onlinePrice || booking.onlinePlotAmount || '',
                creditPoint: booking.creditPoint || booking.creditPointAmount || '',

                onlinePlotAmount: booking.onlinePlotAmount || '',
                creditPointAmount: booking.creditPointAmount || '',
                totalPlotAmount: booking.totalPlotAmount || '',
                bookingAmount: booking.bookingAmount || '0',
                maintenance: booking.maintenance || '0',
                stampDuty: booking.stampDuty || '0',
                legalFees: booking.legalFees || '0',
                ...
                address: booking.address || (booking as any).userAddress || (booking as any).clientAddress || (booking as any).permanentAddress || booking.city || booking.state || '',
                city: booking.city || '',
                state: booking.state || '',
                aadharCardFrontImage: booking.aadharCardFrontImage || null,
                aadharCardBackImage: booking.aadharCardBackImage || null,
                panCardNumber: booking.panCardNumber || null,
            };
        });
    }, [bookingList]);
    ...
    const getBookingPermissionIds = useCallback((): number[] => {
        ...
        if (bookingPerm?.permissionIds?.length > 0) return bookingPerm.permissionIds;
    ...
    const handleAdd = () => { setCurrentBooking(null); setIsModalOpen(true); };
    ...
    const confirmDelete = async () => {
        try {
            if (deleteMode === 'single') {
                ...
                const response = await fetch(`${API_BASE_URL}/bookings/deleteBooking/${id}`, {
                    method: 'DELETE',
                    headers: buildHeaders(true),
                    credentials: 'include',
                });
    ...
    const handleSaveBooking = () => {
        // toast.success(currentBooking ? 'Booking updated successfully' : 'Booking created successfully and sent for admin approval');
    ...
    const handleBulkDelete = (ids: number[]) => {
        if (!ids || ids.length === 0) { toast.error('No bookings selected'); return; }
        setDeleteMode('bulk');
        setDeleteTarget(ids);
        setIsDeleteModalOpen(true);
    };
    ...
    const handleAssignBookings = (bookingIds: number[]) => { setSelectedIds(bookingIds); };
    ...
    return (
        <div className="p-1 md:p-2 bg-gray-50 dark:bg-gray-900 min-h-screen">
            ...
            <BookingModal
                isOpen={isModalOpen}
                onClose={() => { setIsModalOpen(false); setCurrentBooking(null); }}
