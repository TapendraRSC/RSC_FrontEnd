...
const BookingComponent: React.FC = () => {
    ...
    const fetchBookingsData = useCallback(async () => {
        ...
        try {
            ...
            const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8002'}/bookings/getAllBookings?${queryParams.toString()}`;
        ...
        try {
            if (deleteMode === 'single') {
                ...
                const response = await fetch(
                    `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8002'}/bookings/deleteBooking/${id}`,
                    {
                        method: 'DELETE',
                        headers: buildHeaders(true),
                        credentials: 'include',
