...
const BookingModal: React.FC<ComprehensiveLeadModalProps> = ({
  ...
  // --- OPTIMIZED Lead Fetch with Search + Pagination ---
  // Backend handles Admin vs User filtering based on logged-in user's token
  ...
  // Initial load when modal opens - load first 20 leads
  useEffect(() => {
  ...
  const fetchActiveProjects = async () => {
    ...
    try {
      ...
      console.error("Error fetching projects:", error);
      setProjects([]);
      // Show user-facing error only for non-404
      if (error.response?.status !== 404) {
        setApiError("Failed to fetch projects");
      } else {
        setApiError("Projects endpoint not found — check API route");
      }
    } finally {
      setProjectsLoading(false);
    }
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      ...
      // If editing, pre-populate lead in dropdown
      if (initialData.leadId && initialData.name) {
              ...
              <Controller
                ...
                render={({ field }) => (
                  <SearchableLeadDropdown
                    ...
                    placeholder={
                      leadsLoading && !initialLeadsLoaded
                        ? "Loading leads..."
            ...
            <div className="space-y-1.2">
              ...
              <input
                ...
                {...register("cpName", {
                  minLength: {
                    value: 2,
                    message: "CP Name must be at least 2 characters",
                  },
                  maxLength: {
                    value: 50,
                    message: "CP Name cannot exceed 50 characters",
                  },
                })}
                className="w-full h-11 px-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-orange-500"
                placeholder="Mitesh Patel"
