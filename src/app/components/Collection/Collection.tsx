...
const CollectionComponent: React.FC = () => {
    ...
    const handleSave = async (formData: any) => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8002'}/collection/updateCollection/${id}`, {
                method: 'PUT',
    ...
    const fetchProjectsData = useCallback(async () => {
        try {
            const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8002'}/projects/getAllProjects?page=1&limit=100`;
    ...
    const fetchCollectionsData = useCallback(async (overrideTab?: string) => {
        ...
        try {
            ...
            const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8002'}/collection/getAllCollections?${queryParams.toString()}`;
        ...
        try {
            if (deleteMode === 'single') {
                ...
                const response = await fetch(
                    `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8002'}/collection/deleteCollection/${id}`,
                    {
                        method: 'DELETE',
                        headers: getAuthHeaders(),
                        credentials: 'include',
