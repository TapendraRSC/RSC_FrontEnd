...
import {
    ...
    Hash, CreditCard, Download, Ruler, Users, Eye, MapPin, IdCard
} from 'lucide-react';
...
interface Booking {
    ...
    onlinePlotAmount?: string;
    creditPointAmount?: string;
    maintenance?: string;
    stampDuty?: string;
    legalFees?: string;
    budget?: string;
...
const DocumentViewerModal = ({
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-1 overflow-auto p-4 flex items-center justify-center bg-gray-50 dark:bg-gray-800">
                    {isPdf ? (
                        <iframe src={url} className="w-full h-[70vh] rounded-lg" title={title} />
                    ) : (
                        <img
                            src={url}
                            alt={title}
...
const DocPreviewButton = ({
    label, url, colorClass,
}: { label: string; url: string; colorClass: string; }) => {
    ...
    return (
        <div className="relative" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
            <a
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${colorClass}`}
            >
...
const DocumentButtons = ({ booking, onModalClose }: { booking: Booking; onModalClose?: () => void }) => {
    const hasAadharFront = !!booking.aadharCardFrontImage;
    const hasAadharBack = !!booking.aadharCardBackImage;
    const hasPan = !!booking.panCardNumber && booking.panCardNumber.startsWith('http') && booking.panCardNumber.length > 20;

    if (!hasAadharFront && !hasAadharBack && !hasPan) return null;
    ...
    return (
        <div className="flex flex-wrap gap-2">
            {hasAadharFront && (
                <DocPreviewButton
                    label="Aadhar Front"
                    url={booking.aadharCardFrontImage!}
                    colorClass="bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-700 hover:bg-blue-100 dark:hover:bg-blue-900/50"
                />
            )}
            {hasAadharBack && (
                <DocPreviewButton
                    label="Aadhar Back"
                    url={booking.aadharCardBackImage!}
                    colorClass="bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 border-orange-200 dark:border-orange-700 hover:bg-orange-100 dark:hover:bg-orange-900/50"
                />
            )}
            {hasPan && (
                <DocPreviewButton
                    label="PAN Card"
                    url={booking.panCardNumber!}
                    colorClass="bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-700 hover:bg-green-100 dark:hover:bg-green-900/50"
                />
            )}
        </div>
    );
...
const TableDocButton = ({ label, url, title, colorClass }: { label: string; url: string; title: string; colorClass: string }) => {
    return <DocPreviewButton label={label} url={url} colorClass={colorClass} />;
};
...
const getBookingColumns = () => {
    return [
        ...
        { label: 'Gender', accessor: 'gender', sortable: true, minWidth: 80 },
        ...
        { label: 'Total Plot Amt', accessor: 'totalPlotAmount', sortable: true, minWidth: 130 },
        { label: 'Online Amt', accessor: 'onlinePlotAmount', sortable: true, minWidth: 120 },
        { label: 'Credit Amt', accessor: 'creditPointAmount', sortable: true, minWidth: 120 },
        { label: 'Maintenance', accessor: 'maintenance', sortable: true, minWidth: 110 },
        { label: 'Stamp Duty', accessor: 'stampDuty', sortable: true, minWidth: 110 },
        { label: 'Legal Fees', accessor: 'legalFees', sortable: true, minWidth: 110 },
        ...
        { label: 'Address', accessor: 'address', sortable: false, minWidth: 200 },
        { label: 'Aadhar Front', accessor: 'aadharCardFrontImage', sortable: false, minWidth: 120 },
        { label: 'Aadhar Back', accessor: 'aadharCardBackImage', sortable: false, minWidth: 120 },
        { label: 'PAN Card', accessor: 'panCardNumber', sortable: false, minWidth: 110 },
    ];
};
...
const BookingTable: React.FC<BookingTableProps> = ({
    ...
    const formatBookingData = (booking: Booking) => ({
        ...
        formattedOnlineAmount: formatCurrency(booking.onlinePlotAmount),
        formattedCreditAmount: formatCurrency(booking.creditPointAmount),
                                        ...
                                        >
                                            {columns.map((column: any) => {
                                                ...
                                                if (accessor === 'gender') {
                                                    return (
                                                        <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm">
                                                            <span className={`px-2 py-0.5 rounded-full text-xs flex items-center gap-1 w-fit ${booking.gender === 'Male' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' : booking.gender === 'Female' ? 'bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'}`}>
                                                                {booking.gender === 'Male' ? (
                                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                                                        <circle cx="10" cy="14" r="5" /><line x1="19" y1="5" x2="13.6" y2="10.4" /><line x1="19" y1="5" x2="14" y2="5" /><line x1="19" y1="5" x2="19" y2="10" />
                                                                    </svg>
                                                                ...
                                                                {formattedBooking.formattedGender}
                                                            </span>
                                                        </td>
                                                    );
                                                }
                                                if (accessor === 'plotSize') {
                                                    return <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-blue-600 dark:text-blue-400">{formattedBooking.formattedPlotSize}</td>;
                                                }
                                                if (accessor === 'bookingAmount') {
                                                    return <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-green-600 dark:text-green-400">{formattedBooking.formattedBookingAmount}</td>;
                                                }
                                                if (accessor === 'totalPlotAmount') {
                                                    return <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-orange-600 dark:text-orange-400">{formattedBooking.formattedTotalAmount}</td>;
                                                }
                                                if (accessor === 'onlinePlotAmount') {
                                                    return <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-purple-600 dark:text-purple-400">{formattedBooking.formattedOnlineAmount}</td>;
                                                }
                                                if (accessor === 'creditPointAmount') {
                                                    return <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-indigo-600 dark:text-indigo-400">{formattedBooking.formattedCreditAmount}</td>;
                                                }
                                                if (accessor === 'maintenance') {
                                                    return <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-teal-600 dark:text-teal-400">{formattedBooking.formattedMaintenance}</td>;
                                                }
                                                if (accessor === 'stampDuty') {
                                                    return <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-amber-600 dark:text-amber-400">{formattedBooking.formattedStampDuty}</td>;
                                                }
                                                if (accessor === 'legalFees') {
                                                    return <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-rose-600 dark:text-rose-400">{formattedBooking.formattedLegalFees}</td>;
                                                }
                                                // if (accessor === 'address') {
                                                //     const addr = (booking as any).address;
                                                //     return (
                                                //         <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 text-xs sm:text-sm text-gray-600 dark:text-gray-300 max-w-[220px]">
                                                //             {addr ? (
                                                ...
                                                if (accessor === 'address') {
                                                    ...
                                                    return (
                                                        <td
                                                            key={`booking-${booking.id}-${accessor}`}
                                                            className="px-2 sm:px-4 py-2 sm:py-4 text-xs sm:text-sm text-gray-600 dark:text-gray-300 max-w-[300px] whitespace-normal break-words"
                                                        >
                                                            {addr ? (
                                                                <div className="flex items-start gap-1.5">
                                                                    <MapPin className="h-3.5 w-3.5 text-rose-500 mt-0.5 shrink-0" />
                                                ...
                                                if (accessor === 'aadharCardFrontImage') {
                                                    const url = (booking as any).aadharCardFrontImage;
                                                    return (
                                                        <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm">
                                                            {url ? (
                                                                <TableDocButton label="Aadhar Front" url={url} title="Aadhar Card Front" colorClass="bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-700 hover:bg-blue-100" />
                                                            ) : <span className="text-gray-400 text-xs">N/A</span>}
                                                        </td>
                                                    );
                                                }
                                                if (accessor === 'aadharCardBackImage') {
                                                    const url = (booking as any).aadharCardBackImage;
                                                    return (
                                                        <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm">
                                                            {url ? (
                                                                <TableDocButton label="Aadhar Back" url={url} title="Aadhar Card Back" colorClass="bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 border-orange-200 dark:border-orange-700 hover:bg-orange-100" />
                                                            ) : <span className="text-gray-400 text-xs">N/A</span>}
                                                        </td>
                                                    );
                                                }
                                                if (accessor === 'panCardNumber') {
                                                    const url = (booking as any).panCardNumber;
                                                    const isValidUrl = url && url.startsWith('http') && url.length > 20;
                                                    return (
                                                        <td key={`booking-${booking.id}-${accessor}`} className="px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm">
                                                            {isValidUrl ? (
                                                                <TableDocButton label="PAN Card" url={url} title="PAN Card" colorClass="bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-700 hover:bg-green-100" />
                                                            ) : <span className="text-gray-400 text-xs">N/A</span>}
                                                        </td>
                                                    );
                                                }
                ...
                <div className="space-y-3 sm:space-y-4">
                    {leads.length > 0 ? (
                        leads.map(booking => {
                            ...
                            const hasDocs = hasAadharFront || hasAadharBack || hasPanCard;
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="relative p-5 space-y-4">
                                        <div className="flex flex-col sm:flex-row justify-between items-start gap-4 w-full">
                                            <div className="space-y-2 flex-1">
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    {booking.gender && (
                                                        <span className={`px-2 py-0.5 rounded-full text-xs flex items-center gap-1 ${booking.gender === 'Male' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' : booking.gender === 'Female' ? 'bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'}`}>
                                                            {booking.gender === 'Male' ? (
                                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                                                    <circle cx="10" cy="14" r="5" /><line x1="19" y1="5" x2="13.6" y2="10.4" /><line x1="19" y1="5" x2="14" y2="5" /><line x1="19" y1="5" x2="19" y2="10" />
                                                                </svg>
                                            ...
                                            {isCardAdmin && isPending && (
                                                <div className={`flex items-center gap-2 self-end sm:self-start transition-opacity duration-200 ${isHovered ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); handleBookingAction(booking.id, 'approve'); }}
                                                        className="px-4 py-1.5 text-xs font-bold uppercase text-white bg-green-600 hover:bg-green-700 rounded-lg transition-all flex items-center gap-1.5"
                                                    >
                                            ...
                                            {/* {!isPending && booking.approvedByName && booking.bookingDate && (
                                                <div className="flex items-center gap-3 opacity-75 text-sm">
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        <span className={`font-medium ${booking.status?.toLowerCase() === "confirmed" ? "text-green-700 dark:text-green-300" : "text-red-700 dark:text-red-300"}`}>
                                                            {booking.status?.toLowerCase() === "confirmed"
                                        ...
                                        {(booking.address || (booking as any).address) && (
                                            <div className="flex items-start gap-2 text-sm text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/50 rounded-lg px-3 py-2 border border-gray-100 dark:border-gray-700">
                                                <MapPin className="h-4 w-4 text-rose-500 mt-0.5 shrink-0" />
                                                <span className="leading-snug">{booking.address || (booking as any).address}</span>
                                            </div>
                                        )}


                                        {hasDocs && (
                                            <DocumentButtons booking={booking} onModalClose={() => setHoveredId(null)} />
                                        )}


                                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 p-4 rounded-xl border bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 transition-all duration-300 hover:shadow-md hover:-translate-y-0.5">
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm font-bold text-blue-700 dark:text-blue-300">{formattedBooking.formattedPlotSize}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs font-semibold text-orange-600 dark:text-orange-400 flex items-center gap-1 mb-1">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-xl border bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/10 border-green-200 dark:border-green-700 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5">
                                            <div>
                                                <p className="text-xs text-gray-600 dark:text-gray-400 flex items-center mb-1"><IndianRupee className="h-3 w-3 mr-1" /> Booking Amount</p>
                                                <p className="text-lg font-bold text-green-600 dark:text-green-400">{formattedBooking.formattedBookingAmount}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-600 dark:text-gray-400 flex items-center mb-1"><IndianRupee className="h-3 w-3 mr-1" /> Total Plot Amount</p>
                                                <p className="text-lg font-bold text-orange-600 dark:text-orange-400">{formattedBooking.formattedTotalAmount}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-600 dark:text-gray-400 flex items-center mb-1"><IndianRupee className="h-3 w-3 mr-1" /> Online Amount</p>
                                                <p className="text-lg font-bold text-purple-600 dark:text-purple-400">{formattedBooking.formattedOnlineAmount}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-600 dark:text-gray-400 flex items-center mb-1"><CreditCard className="h-3 w-3 mr-1" /> Credit Amount</p>
                                                <p className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{formattedBooking.formattedCreditAmount}</p>
                                            </div>
                                        </div>


                                        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 p-4 rounded-xl border bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/10 border-amber-200 dark:border-amber-700 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5">
                                            <div>
                                                <p className="text-xs text-gray-600 dark:text-gray-400 flex items-center mb-1">Maintenance</p>
                                                <p className="text-sm font-bold text-teal-600 dark:text-teal-400">{formattedBooking.formattedMaintenance}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-600 dark:text-gray-400 flex items-center mb-1">Stamp Duty</p>
                                                <p className="text-sm font-bold text-amber-600 dark:text-amber-400">{formattedBooking.formattedStampDuty}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-600 dark:text-gray-400 flex items-center mb-1">Legal Fees</p>
                                                <p className="text-sm font-bold text-rose-600 dark:text-rose-400">{formattedBooking.formattedLegalFees}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-600 dark:text-gray-400 flex items-center mb-1">Payment Ref</p>
                                                <p className="text-sm font-bold text-gray-700 dark:text-gray-300">{booking.payment_reference || 'N/A'}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-600 dark:text-gray-400 flex items-center mb-1">Payment Platform</p>
                                                <p className="text-sm font-bold text-gray-700 dark:text-gray-300">{booking.paymentPlatformName || 'N/A'}</p>
                                            </div>
                                        </div>
                                        ...
                                        {booking.remark && booking.remark !== 'N/A' && (
                                            <div className="px-3 py-2 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-700">
                                                <p className="text-[10px] sm:text-xs text-yellow-700 dark:text-yellow-300">
                                                    <span className="font-semibold">Remark:</span> {booking.remark}
                                                </p>
                                            </div>
                                        )}
